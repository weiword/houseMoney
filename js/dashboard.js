...
(function () {
  ...
  var newsData = null, newsError = null;
  ...
  function newsHtml() {
    var head = '<h1 class="page-title">News</h1><div class="page-sub">Crypto &amp; markets \u00b7 updated hourly</div>';
    if (newsData && newsData.items && newsData.items.length) {
      var rows = newsData.items.map(function (it) {
        return '<a class="news-item" href="' + esc(it.link) + '" target="_blank" rel="noopener">'
          + '<div class="news-meta">' + esc(it.source) + (it.date ? ' \u00b7 ' + timeAgo(it.date) : '') + '</div>'
          + '<div class="news-title">' + esc(it.title) + '</div>'
          + (it.summary ? '<div class="news-sum">' + esc(it.summary) + '</div>' : '')
          + '</a>';
      }).join('');
      return head + '<div class="news-feed">' + rows + '</div>';
    }
    if (newsError) return head + '<div class="tbl-wrap"><div class="note">' + esc(newsError) + '</div></div>';
    return head + '<div class="tbl-wrap"><div class="note">Loading news\u2026</div></div>';
  }
  function reNews() { if (activePage === 'news' && content()) content().innerHTML = newsHtml(); }
  async function ensureNews() {
    if (newsData) { reNews(); return; }
    try {
      var res = await fetch('news.json?_=' + Date.now(), { cache: 'no-store' });
      if (!res.ok) throw new Error(res.status === 404 ? 'The news feed will appear once the updater has run once.' : 'HTTP ' + res.status);
      newsData = await res.json();
      reNews();
    } catch (e) { newsError = (e && e.message) ? e.message : 'Could not load news.'; reNews(); }
  }
  ...
  function renderPage(key) {
    ...
    if (key === 'news') { c.innerHTML = newsHtml(); ensureNews(); return; }
